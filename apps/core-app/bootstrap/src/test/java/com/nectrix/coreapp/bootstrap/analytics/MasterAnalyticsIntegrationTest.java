...
/**
 * TICKET-116 — MasterAnalyticsController's real equity-curve/monthly-returns/P&amp;L-by-instrument
 * computation against seeded account_snapshots/copied_trades/trade_signals rows, plus the ownership
 * gate ({@code MasterAnalyticsService#getOwnedMasterProfile}'s own {@code @PostAuthorize}). Same
 * real-HTTP-round-trip style as MasterProfileIntegrationTest/ CopyRelationshipIntegrationTest.
 */
@Tag("integration")
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
class MasterAnalyticsIntegrationTest {
  ...
  private NewUser newUser(String... roles) {
    String email = "master-analytics-" + UUID.randomUUID() + "@example.com";
    UUID userId =
  ...
  private record RelationshipChain(UUID relationshipId, UUID followerBrokerAccountId) {}

  /** Minimal chain to seed a closed, realized-P&L copied_trades row for pnlByInstrument. */
  private RelationshipChain insertClosedTradeChain(
    ...
    return new RelationshipChain(relationshipId, followerBrokerAccountId);
  }

  @Test
  void analytics_returnsRealEquityCurveAndPnlByInstrument_forTheOwningMaster() {
    ...
    insertSnapshot(masterBrokerAccountId, now.minus(5, ChronoUnit.DAYS), 10000.0);
    insertSnapshot(masterBrokerAccountId, now.minus(1, ChronoUnit.DAYS), 11000.0);

    insertClosedTradeChain(
        masterProfileId, masterBrokerAccountId, "EURUSD", new BigDecimal("250.00"), now);

    HttpResult response =
        request(
            "GET",
            "/api/v1/master-profiles/" + masterProfileId + "/analytics?period=ALL",
            master.accessToken());

    assertThat(response.status()).isEqualTo(200);
    List<Map<String, Object>> equityCurve =
        (List<Map<String, Object>>) response.body().get("equity_curve");
    assertThat(equityCurve).hasSizeGreaterThanOrEqualTo(2);
    ...
    assertThat(pnlByInstrument.get(0).get("total_pnl")).isEqualTo(250.0);
  }

  @Test
  void analytics_withNoEquityDataAtAll_returnsEmptyCurveNotAnError() {
    NewUser master = newUser("MASTER");
    UUID masterBrokerAccountId = insertBrokerAccount(master.userId());
    UUID masterProfileId = insertMasterProfile(master.userId(), masterBrokerAccountId);
    ...
    HttpResult response =
        request(
            "GET",
            "/api/v1/master-profiles/" + masterProfileId + "/analytics?period=30D",
            master.accessToken());
  ...
  @Test
  void analytics_forAnotherMastersProfile_isForbidden() {
    NewUser master = newUser("MASTER");
    UUID masterBrokerAccountId = insertBrokerAccount(master.userId());
    UUID masterProfileId = insertMasterProfile(master.userId(), masterBrokerAccountId);
    ...
    HttpResult response =
        request(
            "GET",
            "/api/v1/master-profiles/" + masterProfileId + "/analytics",
  ...
  @Test
  void analytics_forANonexistentMasterProfile_returns404() {
    NewUser master = newUser("MASTER");

    HttpResult response =
        request(
            "GET",
            "/api/v1/master-profiles/" + UUID.randomUUID() + "/analytics",
            master.accessToken());
