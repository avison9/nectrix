...
- `go/` — a standalone Go module. Generated code lives in `go/gen/` and is checked in (standard Go convention — no build-time codegen step). `go/eventconsumer/` is the producer/idempotent-consumer helper package; its default dedup implementation now comes from `packages/redis-client/go` (see below).
...
**Idempotency-dedup default (TICKET-008)**: both languages now delegate to the shared `packages/redis-client/{java,go}` library rather than carrying their own dedup implementation. Go's `redisclient.Deduper` implements `packages/go-domain`'s `Deduper` interface (that interface's own comment originally anticipated this: "the real Redis-backed implementation lands in TICKET-008"); Java's `IdempotentConsumer` is constructed with `com.nectrix.redisclient.RedisDeduplicator`. Both are thin `SET key val NX EX ttl` wrappers — see `packages/redis-client/README.md` for the full design (including the "Redis is never system-of-record" rule and the cluster-vs-standalone client duality).
