.PHONY: up down restart logs ps clean \
	...
	kind-netpol-test kind-hpa-test kind-mt-terminal-host-test \
...
go-build: ## Build all Go modules (go-domain, event-contracts/go, redis-client/go, ctrader-proto/go, and the four Go apps)
	...
	$(DC_EXEC) bash -c "cd /workspace && for d in packages/go-domain packages/event-contracts/go packages/redis-client/go packages/ctrader-proto/go apps/copy-engine apps/broker-adapters apps/mt5-bridge-gateway apps/mt-terminal-host; do (cd \$$d && go build -o /dev/null ./...); done"

go-test: ## Test all Go modules (excludes //go:build integration-tagged tests)
	$(DC_EXEC) bash -c "cd /workspace && for d in packages/go-domain packages/event-contracts/go packages/redis-client/go packages/ctrader-proto/go apps/copy-engine apps/broker-adapters apps/mt5-bridge-gateway apps/mt-terminal-host; do (cd \$$d && go test ./...); done"

go-lint: ## Lint all Go modules with golangci-lint
	$(DC_EXEC) bash -c "cd /workspace && for d in packages/go-domain packages/event-contracts/go packages/redis-client/go packages/ctrader-proto/go apps/copy-engine apps/broker-adapters apps/mt5-bridge-gateway apps/mt-terminal-host; do (cd \$$d && golangci-lint run ./...); done"
...
kind-mt-terminal-host-test: ## Real kind run proving mt-terminal-host's reconcile loop + RBAC scoping
	./infra/kind/mt-terminal-host-test/run.sh
